...
class mesh2D():
	...
	def buildMesh(self):
		...
		for j in range(self.M):
			for i in range(self.N):
				self.x[i + self.N*j, ...], self.y[i + self.N*j, ...] = self.get_mapping(self.XI, self.ETA, i, j);
				self.tx[i + self.N*j, ...], self.ty[i + self.N*j, ...] = self.get_mapping(self.tXI, self.tETA, i, j);
				self.x_hiDOP[i + self.N*j, ...], self.y_hiDOP[i + self.N*j, ...] = self.get_mapping(self.XI_hiDOP, self.ETA_hiDOP, i, j);
	...
	def plotMesh(self):
		...
		for i in range(self.x.shape[0]):
			ax.plot(self.x[i, ...], self.y[i, ...], 'r-o', linewidth = 0.5, label = 'Primal mesh', markersize = 2);
			ax.plot(self.tx[i, 1:-1, 1:-1], self.ty[i, 1:-1, 1:-1], 'b-s', linewidth = 0.5, label = 'Dual mesh', markersize = 2);
			ax.plot(self.x[i, ...].T, self.y[i, ...].T, 'r-', linewidth = 0.5);
			ax.plot(self.tx[i, 1:-1, 1:-1].T, self.ty[i, 1:-1, 1:-1].T, 'b-', linewidth = 0.5);
		# texts = [ax.text(X, Y, str(c), color = 'r', fontsize = 12) for c, (X, Y) in enumerate(zip(self.x.flatten(), self.y.flatten()))];
		# adjust_text(texts);
		...
		ax.set_ylabel(r'$y$');
		ax.set_aspect('equal');
		ax.legend(handles, labels, bbox_to_anchor = (0., 1.01, 1., .102), loc = 'lower left',
           		ncol = 2, mode = 'expand', borderaxespad = 0., fontsize = 10);
		# plt.savefig('./Mesh.pdf', dpi = 300, bbox_inches = 'tight');
		# plt.show();
		return ax;